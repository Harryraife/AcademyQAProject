...
def update_task(id):
    ...
    form = NewLift()
#    lifter = Lifter.query.all()
#    log.lifter_id.choices = [(lifter.id, f"{user.forename} {user.surname}") for user in users]
    if form.validate_on_submit():
        lift_to_update.date = form.date.data
        lift_to_update.weight = form.weight.data
        lift_to_update.reps = form.reps.data
        lift_to_update.lift = form.lift.data
        db.session.commit()
        return redirect(url_for('view_all_tasks'))
    form.lift.data = lift_to_update.lift
